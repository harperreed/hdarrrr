...
func validateImageProperties(images []image.Image) error {
    ...
    // Use PNG as default format for validation since we're working with in-memory images
    baseProps := imaging.GetImageProperties(images[0], ".png")
    for i, img := range images[1:] {
        props := imaging.GetImageProperties(img, ".png")
