...
func main() {
    ...
    // Validate HDR method
    if err := validateHDRMethod(*hdrMethod); err != nil {
        log.Fatal("Error: ", err)
    }
    ...
    output, err := hdrProcessor.Process(alignedImages, *hdrMethod)
    if err != nil {
...
func validateHDRMethod(method string) error {
    validMethods := map[string]bool{
        "tone-mapping":      true,
        "exposure-fusion":   true,
    }
    
    if !validMethods[method] {
        return fmt.Errorf("unsupported HDR method: %s. Supported methods: tone-mapping, exposure-fusion", method)
    }
    return nil
}
